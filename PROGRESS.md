...
|------|--------|--------|------|
| 测试用例数 | 21 | 21 | — |
| 准确率 (Accuracy) | **19.0% (4/21)** | **19.0% (4/21)** | Δ = 0 |
| RAG 检索命中 | — | 1-2 hits/query | 向量检索正常 |

**根因分析**: 评测用 4-5 句上下文片段（非完整章节），agent 无法获取足够信息判断说话人。以案例 A1 为例，RAG 成功将预测从 "里面的男人" 纠正为 "石头见" ✅——证明 RAG 检索到的人物信息帮助了 agent，但整体准确率受限于上下文片段太小，不是 RAG 的问题。
